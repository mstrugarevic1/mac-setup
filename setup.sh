...
# Touch ID for sudo via the sudo_local drop-in (macOS Sonoma and newer).
# Best-effort: a missing template or unexpected format must never abort setup,
# because this is a convenience, not a required part of the workstation.
...
# Ensure the login shell is zsh. $SHELL reflects the current process, not the
# configured login shell, so read it from the Directory Service record instead.
